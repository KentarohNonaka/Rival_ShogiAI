...
#以下変更
    ...
    # 手加減手法の実装　
    # 相手の平均勝率に近い手から選択確率が最も高い手を選ぶ(該当なしの場合，選択確率+勝率が最も高い手を優先)
    def get_bestmove_and_print_pv(self):
        ...
        #print(win_rate_diff[selected_index])

        #勝率差が少ない手がない場合，すべての候補手から選択確率+勝率が最大の手を選択
        if win_rate_diff[selected_index] > 0.1:
            ...
            #print(f'policy_list\n{policy_list}\n')
            win_plus_policy = win_rate + policy_list
            #print(f'win_plus_policy\n{win_plus_policy}\n')
            selected_index = np.nanargmax(win_plus_policy)
